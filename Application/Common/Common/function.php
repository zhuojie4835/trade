 ...
 */
function getRedis() {
    static $redis_handle = false;

    if ($redis_handle) return $redis_handle;

    \Predis\Autoloader::register();
	$config = C('REDIS_CONFIG');
    $redis_handle = new \Predis\Client($config);
    return $redis_handle;
}
 ...
 * 生成流水号
 */
function generateFollowNumber($type='T') {
	$number = $type . time() . rand(1000,9999);
	$redis = getRedis();
	if($redis->exists('follow_number') && $redis->sismember('follow_number',$number)) {
		generateFollowNumber($type);
	}
	$redis->sadd('follow_number',$number);
	return $number;
}
 ...
 * 获取产品价格信息
 */
function getProductTradeInfo($pid='') {
	$redis = getRedis();
	if(!$product_trade_info = $redis->hgetall('product_trade:'.$pid)) {
		if(!$product = $redis->hgetall('product:'.$pid)) {
			return null;
		}
		$product = unserialize($product['product_info']);
		$total_number = $product['issue_number'];
		$price_limit = isset($product['price_limit']) ? (int)$product['price_limit'] : 0.1;//涨跌幅未设置默认
		$now_price = $product['issue_price'];
		$open_price = $product['issue_price'];
		$close_price = $product['issue_price'];
		$low_price = $product['issue_price'];
		$high_price = $product['issue_price'];
		$min_price = sprintf('%.2f',(1-$price_limit)*$open_price);
		$max_price = sprintf('%.2f',(1+$price_limit)*$open_price);
		$product_trade_info = array(
		...
		$redis->hmset('product_trade:'.$pid,$product_trade_info);
	}
	
	return $product_trade_info;
}

/**
 * 返回小数
 */
 function getFloat($number,$jindu=2) {
	return sprintf('%.'.$jindu.'f',$number); 
 }
 ...
 * author:zhuojie
 */
function arr_sort($sort_arr,$arr) {
    $arrSort = array();  
    ...
    if($sort_arr['direction']){  
        array_multisort($arrSort[$sort_arr['field']], constant($sort_arr['direction']), $arr);  
    }
